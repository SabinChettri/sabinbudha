...
const STACK_LAYERS = [
  { label: "UI / INTERFACE", detail: "React · Next.js · Tailwind", tint: "signal" },
  { label: "APPLICATION LAYER", detail: "Node.js · Express · REST", tint: "amber" },
  { label: "DATA LAYER", detail: "PostgreSQL · MongoDB · Prisma", tint: "signal" },
];
...
export default function Hero() {
  return (
    ...
    >
      <div className="pointer-events-none absolute -top-40 left-1/2 h-[420px] w-[720px] -translate-x-1/2 rounded-full bg-signal-500/10 blur-[120px]" />

      <div className="section grid grid-cols-1 items-center gap-16 py-0 lg:grid-cols-[1.05fr_0.95fr]">
        {/* Left column */}
        <div className="animate-fade-up">
          <span className="eyebrow">
            <Circle size={7} className="fill-signal-500 text-signal-500 animate-pulse-dot" />
            Full-Stack Developer &amp; UI Specialist
          </span>

          <h1 className="mt-6 max-w-xl font-display text-4xl font-medium leading-[1.08] tracking-tight text-fg text-balance sm:text-5xl md:text-6xl [html:not(.dark)_&]:text-fg-light">
            From 3D Frontend Interfaces To Full&#8209;Stack Systems.
          </h1>

          <p className="section-sub mt-6 max-w-lg">
            Modern web applications, scalable backend architecture, and
            high-performance digital experiences built with clean code and
          ...
          <div className="mt-14 flex flex-wrap items-center gap-x-10 gap-y-4 font-mono text-xs uppercase tracking-wider text-fg-faint [html:not(.dark)_&]:text-fg-light-muted">
            <span>05+ Production Systems</span>
        ...
        {/* Right column — signature visual: laptop preview + floating system panel */}
        <div className="relative mx-auto w-full max-w-md pb-14 animate-fade-up [animation-delay:150ms] sm:pb-8">
            ...
            <div className="rounded-t-2xl border border-border bg-ink-900 p-2 shadow-2xl [html:not(.dark)_&]:border-border-light [html:not(.dark)_&]:bg-gray-100">
              <div className="overflow-hidden rounded-lg border border-border bg-ink-950 [html:not(.dark)_&]:border-border-light [html:not(.dark)_&]:bg-white">
                ...
                <div className="space-y-3 p-4">
                  ...
                  <div className="flex h-24 items-end gap-1.5 rounded-lg border border-border bg-ink-800/70 p-3 [html:not(.dark)_&]:border-border-light [html:not(.dark)_&]:bg-gray-100">
                    {CHART_BARS.map((h, i) => (
                      <div
                        key={i}
                        className={`flex-1 rounded-sm ${
                          i % 3 === 0 ? "bg-amber-500/70" : "bg-signal-500/60"
                        }`}
                        style={{ height: `${h}%` }}
                      />
                    ))}
          ...
          {/* Floating system-status panel, overlapping the laptop */}
          <div className="absolute -bottom-2 -left-3 w-[15.5rem] rotate-[-3deg] rounded-xl border border-border bg-surface p-3.5 shadow-2xl transition-transform duration-300 hover:rotate-0 sm:-bottom-8 sm:-left-6 sm:w-64 [html:not(.dark)_&]:border-border-light [html:not(.dark)_&]:bg-surface-light">
            <div className="flex items-center justify-between border-b border-border pb-2.5 [html:not(.dark)_&]:border-border-light">
              ...
              <Circle size={6} className="fill-signal-500 text-signal-500 animate-pulse-dot" />
            </div>
                ...
                <div key={layer.label} className="flex items-center justify-between">
                  ...
                  <span
                    className={`h-1.5 w-1.5 shrink-0 rounded-full ${
                      layer.tint === "signal" ? "bg-signal-500" : "bg-amber-500"
                    }`}
                  />
                </div>
              ))}
          ...
          <div className="pointer-events-none absolute -right-6 -top-6 h-32 w-32 rounded-full bg-amber-500/10 blur-2xl" />
        </div>
      </div>
    </section>
  );
